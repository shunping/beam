...
def majority_vote(
    outlier_label=1, normal_label=0, tie_breaker=0, include_history=False):
  def get_vote(decisions: Iterable[AnomalyDecision]) -> AnomalyDecision:
    ...
    if counters[normal_label] < counters[outlier_label]:
      ...
      vote = tie_breaker
    info = (
        '[' + ('; '.join(map(str, decisions))) + ']') if include_history else ''

    return AnomalyDecision(model="aggregate", prediction=vote, info=info)
...
def average_score(include_history=False):
  def get_score(decisions: Iterable[AnomalyDecision]) -> AnomalyDecision:
    ...
    return AnomalyDecision(
        model="aggregate", score=statistics.mean(scores), info=info)
